...
func main() {
	...
	for _, host := range cfg.Hosts {
		...
		for taskIdx := 0; taskIdx < len(taskQueue); taskIdx++ {
			...
			if err != nil {
				fmt.Printf("    ✗ Failed to resolve loop: %v\n", err)
				continue
			}
			...
			if loopSpec != nil {
				if task.IncludeTasks != nil {
					if err := includeTasksForLoop(task, loopSpec, flattened, baseDir, &taskQueue, taskIdx, renderImportPath); err != nil {
						fmt.Printf("    ✗ include_tasks: %v\n", err)
					}
					continue
				}

				if len(loopSpec.Items) == 0 {
					fmt.Printf("    ↷ SKIPPED (empty loop)\n")
					if task.Register != "" {
...
func runInit(args []string) error {
	...
	if err := writeInitFile(readmePath, initReadme, *force); err != nil {
		return err
	}
...
const initReadme = `# Dibra Starter

This directory was generated by dibra init. It contains a minimal CUE
playbook and inventory wired to the local integration-test container.
...
- Edit inventory.cue to define additional hosts or groups.
- Run dibra -config ./deploy.cue -v for verbose output.
`
