...
func TestPlaybook_DockerConfigParity(t *testing.T) {
	client := getClient(t)
	defer client.Close()

	prefix := fmt.Sprintf("dibra-dc-%d", time.Now().UnixNano()%100000000)
	...
	mustRemote(t, client, "rm -f /tmp/dibra-dc-*.json /tmp/.dibra-agent "+dataFile)
	defer func() {
	...
	templatePath := writeResultTemplate(t, "config_result")
	run := func(t *testing.T, name, arguments, taskOptions string) (map[string]any, string) {
		t.Helper()
		remotePath := "/tmp/dibra-dc-" + name + ".json"
...
`
	...
	inspect := func(t *testing.T, ref string) map[string]any {
		t.Helper()
		raw := strings.TrimSpace(mustRemote(t, client, "docker config inspect "+ref+" --format '{{json .}}'"))
	...
	exists := func(ref string) bool {
		t.Helper()
		output := remoteExec(t, client, "docker config inspect "+ref+" >/dev/null 2>&1; echo $?")
	...
	t.Run("invalid template_driver", func(t *testing.T) {
		_, output := run(t, "bad-driver", "      name: foo\n      data: x\n      template_driver: not a template driver\n", "")
		if !strings.Contains(output, "FAILED") || !strings.Contains(output, "value of template_driver must be one of: golang, got: not a template driver") {
			t.Fatalf("invalid driver = %s", output)
		}
	})
	...
	t.Run("check mode does not create", func(t *testing.T) {
		...
		if predicted["changed"] != true || predicted["config_name"] != passwordName {
			t.Fatalf("check create = %#v", predicted)
		}
		if predicted["config_id"] != nil && predicted["config_id"] != "" {
			t.Fatalf("check create must omit config_id: %#v", predicted["config_id"])
		}
		if exists(passwordName) {
			t.Fatal("check mode created a config")
		}
	})
	...
	t.Run("create inspect idempotent data_src and base64", func(t *testing.T) {
		...
		if created["changed"] != true || created["config_name"] != passwordName {
			t.Fatalf("create = %#v", created)
		}
		configID, _ = created["config_id"].(string)
		if configID == "" {
			t.Fatalf("missing config_id: %#v", created)
		}
		engine := inspect(t, configID)
		...
		if idempotent["changed"] != false || idempotent["config_id"] != configID {
	...
	t.Run("data change recreates", func(t *testing.T) {
		...
		if updated["changed"] != true || updated["config_id"] == configID {
			t.Fatalf("update = %#v previous=%s", updated, configID)
		}
		if exists(configID) {
			t.Fatal("old config id should be gone after non-rolling update")
		}
		configID, _ = updated["config_id"].(string)
	})
...
`, "")
	...
	t.Run("force recreates", func(t *testing.T) {
		...
		if forced["changed"] != true || forced["config_id"] == before["config_id"] {
	...
	t.Run("empty data is rejected by Engine 29", func(t *testing.T) {
		...
		if !strings.Contains(output, "FAILED") || !strings.Contains(output, "config data must be larger than 0") {
...
`, "")
	...
	t.Run("missing ansible_key without force is unchanged", func(t *testing.T) {
		mustRemote(t, client, "printf '%s' 'plain' | docker config create "+keylessName+" -")
	...
	t.Run("rolling versions", func(t *testing.T) {
		...
		if first["changed"] != true || first["config_name"] != rollingName+"_v1" {
			t.Fatalf("rolling v1 = %#v", first)
		}
		engine := inspect(t, first["config_id"].(string))
		labels := nestedMap(engine, "Spec", "Labels")
		...
		if second["changed"] != true || second["config_name"] != rollingName+"_v2" || second["config_id"] == first["config_id"] {
			t.Fatalf("rolling v2 = %#v", second)
		}
		if !exists(first["config_id"].(string)) || !exists(second["config_id"].(string)) {
			t.Fatal("rolling update should keep the previous version by default")
		}

		removed := success(t, "roll-absent", "      name: "+rollingName+"\n      rolling_versions: true\n      state: absent\n", "")
		if removed["changed"] != true || exists(first["config_id"].(string)) || exists(second["config_id"].(string)) {
	...
	t.Run("versions_to_keep", func(t *testing.T) {
		...
		if second["config_name"] != keepName+"_v2" {
			t.Fatalf("keep v2 = %#v", second)
		}
		listed := strings.TrimSpace(mustRemote(t, client, "docker config ls --format '{{.Name}}' --filter name="+keepName))
		if listed != keepName+"_v2" {
			t.Fatalf("versions_to_keep=1 listed %q", listed)
		}

		keepAll := success(t, "keep-all", "      name: "+keepName+"\n      data: c\n      rolling_versions: true\n      versions_to_keep: -1\n", "")
		if keepAll["config_name"] != keepName+"_v3" {
			t.Fatalf("keep -1 = %#v", keepAll)
		}
		listed = strings.TrimSpace(mustRemote(t, client, "docker config ls --format '{{.Name}}' --filter name="+keepName+" | sort"))
	...
	t.Run("template_driver golang", func(t *testing.T) {
		success(t, "tpl-plain", "      name: "+templateName+"\n      data: opensesame!\n", "")
		withDriver := success(t, "tpl-golang", "      name: "+templateName+"\n      data: opensesame!\n      template_driver: golang\n", "")
		if withDriver["changed"] != true {
			t.Fatalf("add golang = %#v", withDriver)
		}
		again := success(t, "tpl-golang-idempotent", "      name: "+templateName+"\n      data: opensesame!\n      template_driver: golang\n", "")
		if again["changed"] != false {
			t.Fatalf("golang idempotent = %#v", again)
		}

		templated := success(t, "tpl-data", "      name: "+templateName+"\n      data: e3sgLlNlcnZpY2UuTmFtZSB9fQ==\n      data_is_b64: true\n      template_driver: golang\n", "")
		if templated["changed"] != true {
			t.Fatalf("template data = %#v", templated)
		}
		engine := inspect(t, templated["config_id"].(string))
		spec := nestedMap(engine, "Spec")
		dataStr, _ := spec["Data"].(string)
		decoded, err := base64.StdEncoding.DecodeString(dataStr)
		if err != nil {
			decoded, err = base64.URLEncoding.DecodeString(dataStr)
		}
		if err != nil || string(decoded) != "{{ .Service.Name }}" {
			t.Fatalf("template Data = %q decoded=%q err=%v", dataStr, decoded, err)
		}
		templating := nestedMap(engine, "Spec", "Templating")
		if templating["Name"] != "golang" {
			t.Fatalf("Templating = %#v", templating)
		}
		success(t, "tpl-absent", "      name: "+templateName+"\n      state: absent\n", "")
	})
  ...
  - name: Attach config to a service
    community.docker.docker_swarm_service:
      name: ` + serviceName + `
      image: alpine:latest
      command: ["sleep", "infinity"]
      configs:
        - config_name: ` + inuseName + `_v1
          filename: /tmp/inuse
`
		...
		if updated["changed"] != true || updated["config_name"] != inuseName+"_v2" {
			t.Fatalf("rolling while in use = %#v", updated)
		}
		if !exists(first["config_id"].(string)) {
