...
func extractTask(v cue.Value) (config.Task, error) {
	...
	if whenVal := v.LookupPath(cue.ParsePath("when")); whenVal.Exists() {
		data, err := whenVal.MarshalJSON()
		if err != nil {
			return t, fmt.Errorf("failed to extract task when: %w", err)
		}
		var w config.When
		if err := json.Unmarshal(data, &w); err != nil {
			return t, fmt.Errorf("failed to decode task when: %w", err)
		}
		t.When = w
	}
